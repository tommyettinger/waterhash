...
static inline uint64_t woothash(const void* key, uint64_t len, uint64_t seed) {
    ...
    {
        // Runs 4 different CMR random number generators, each modified by a different part of the hashed data.
        a = (_wootr64(p) ^ a) * _wootp1; a = ROTL64(a, 25);
        b = (_wootr64(p + 8) ^ b) * _wootp2; b = ROTL64(b, 27);
        c = (_wootr64(p + 16) ^ c) * _wootp3; c = ROTL64(c, 29);
        d = (_wootr64(p + 24) ^ d) * _wootp4; d = ROTL64(d, 31);
        // After four 64-bit values are read in and randomized a little with one _wootp constant, they are each
        // multiplied by different _wootp constants, XORed, and added to seed.
        seed += a * _wootp3 ^ b * _wootp4 ^ c * _wootp5 ^ d * _wootp1;
    }
    seed += _wootp5;
    switch (len & 31) {
